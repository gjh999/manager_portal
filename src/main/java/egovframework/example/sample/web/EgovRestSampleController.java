...
import egovframework.example.cmmn.web.EgovRequestWrapper;
...
public class EgovRestSampleController {
	...
	/**
	 * 테스트
	 * @param encData - 암호화 데이터
	 * @param apikey - 헤더에 담긴 apiKey
	 * @param apikey - 헤더에 담긴 apiUtlinsttCode
	 * @return
	 * @exception Exception
	 */
	
	@RequestMapping(value = "/receptapis/responseIssue.do", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<String> testSample2(HttpServletRequest request) throws Exception {
		...
		String insttId = request.getHeader("apiUtlinsttCode");
		System.out.println("insttId = " + insttId);
		
		Map<String, Object> resultMap = sampleService.selectResponseIssue(encData, apiKey, insttId);

		System.out.println("테스트 끝");
		
		String rspnsCode 	= resultMap.get("rspnsCode").toString();
		String rspnsMssage 	= resultMap.get("rspnsMssage").toString();
		String rspnsEncData = resultMap.get("encData").toString();
		
		String result = "";
		
		/* 모두 String Return 방식 */
		Map<String, Object> outputMap = new HashMap<String, Object>();
		outputMap.put("encData", rspnsEncData);
